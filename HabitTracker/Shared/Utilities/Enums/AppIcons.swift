//
//  AppIcons.swift
//  HabitTracker
//
//  Created by Mayur Shrivas on 01/09/25.
//

import SwiftUI
import Foundation

struct AppIcons {
    static let arc = "icon.book"
    static let calendar = "icon.date"
    static let camera = "icon.camera"
    static let check = "icon.stop"
    static let clock = "icon.clock"
    static let coins = "icon.money"
    static let cup = "icon.water"
    static let diamond = "icon.diamond"
    static let dumbell = "icon.gym"
    static let flame = "icon.fire"
    static let fruit = "icon.food"
    static let hand = "icon.handshake"
    static let hat = "icon.scholar"
    static let leaf = "icon.leaf"
    static let lightbulb = "icon.light"
    static let lotus = "icon.peace"
    static let mic = "icon.mic"
    static let moon = "icon.moon"
    static let music = "icon.music"
    static let paint = "icon.paint"
    static let pen = "icon.write"
    static let phone = "icon.phone"
    static let pill = "icon.pill"
    static let rocket = "icon.rocket"
    static let shield = "icon.shield"
    static let shoe = "icon.shoe"
    static let smile = "icon.smile"
    static let star = "icon.star"
    static let sunrise = "icon.sun"
    static let tooth = "icon.tooth"
    static let verified = "icon.verified"
    static let wallet = "icon.business"
    
    static let all: [String] = [
        tooth, cup , fruit , dumbell, shoe, lotus, moon , leaf,
        pill, wallet ,paint , lightbulb , flame , hat , mic, calendar,
        phone, coins , camera, music , smile, shield , pen , sunrise,
        verified, check , rocket , star , clock , diamond , arc , hand
    ]
}



